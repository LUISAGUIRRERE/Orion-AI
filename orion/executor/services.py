...
def _resolve_adapter_name(mission: Mission) -> str:
    ...
    picks the adapter explicitly; absent that, the deterministic local
    adapter is the default -- the only adapter this Sprint ships. This
    keeps adapter selection real (not hypothetical) without inventing
    a new Mission field before a second real adapter exists to justify
    one.
    """
...
def run_for_mission(mission: Mission, repo_root: Path) -> tuple[list[str], str]:
    ...
    if result.status != ExecutionStatus.SUCCEEDED:
        bridge_services.record_event(
            mission.id,
            "execution_failed",
            result.error or f"El adaptador termino con estado {result.status.value}.",
            AUTHOR,
        )
        raise ExecutorError(
...
def _run_with_timeout(
    ...
    """Call ``adapter.execute(request)`` bounded by
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(adapter.execute, request)  # type: ignore[attr-defined]
